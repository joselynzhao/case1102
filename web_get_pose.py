...
@click.option('--encoder', default='./case1103_2_02/network-snapshot-000050.pkl')
...
@click.option("--start_point", type=int, default=0)
@click.option("--end_point", type=int, default=10000)
@click.option("--local_rank", type=int, default=0)
@click.option("--out_name", type=str, default='case1103_2_02_3')

def main(g_ckpt, batch, local_rank, encoder, which_server,out_name,start_point,end_point):
    # local_rank = rank
    ...
    data ='/workspace/datasets/web_car/'
    input_images = os.path.join(data,'data')
    ...
    images.sort()   # 原始的image_name
    print("len of images is ",len(images))
    if end_point >len(images):
        images = images[start_point:]
    else:
        images = images[start_point:end_point]
    ...
    for idx,image in enumerate(tqdm.tqdm(images)):
        ...
        try:
            with bf.BlobFile(os.path.join(input_images,image), "rb") as f:
                pil_image = PIL.Image.open(f)
                pil_image.load()
                idx_c = idx

                pil_image = pil_image.convert("RGB")
                pil_image = pil_image.resize((resolution, resolution), resample=PIL.Image.BICUBIC)
                ...
                image_id = images[idx_c].split('.')[0]  # 去掉后缀
                ...
                np.savez(os.path.join(pose_dir, f'{image_id}.npz'), camera_0=camera_0, camera_1=camera_1, camera_2=camera_2)

                # import shutil
                # dst_name = os.path.join(img_dir, f'{image_id}.jpg')
                # src_name = img_path
