...
@click.option("--case", type=int, default=0)  # 0 or 1
...
@click.option("--outdir", type=str, default='./output/case1020_B/debug')
...
def main(outdir, g_ckpt, e_ckpt,
         ...
         lambda_img, lambda_l2, which_c, adv, tensorboard, resume, which_server,which_camera,case):
    # local_rank = rank
    # setup(rank, word_size)
    # options_list = click.option()
    # print(options_list)
    data1 = data_path[which_server]['data1']  # 只使用data1 进行训练。
    ...
    if case == 0:
        print("training model using case 0")
        for idx in pbar:
            i = idx + start_iter
            if i > max_steps:
                print("Done!")
            ...
            # mapping_w += ws_avg
            loss_dict['loss_w'] = F.smooth_l1_loss(rec_ws, gt_w).mean()
        ...
        print("training model using case 1")
        for idx in pbar:
            i = idx + start_iter
            if i > max_steps:
                print("Done!")
            ...
            rec_ws += ws_avg
            gen_img = G.get_final_output(styles=rec_ws, camera_matrices=camera_matrices)
            loss_dict['img1_lpips'] = loss_fn_alex(img.cpu(), gen_img.cpu()).mean().to(device)
            # mapping_w += ws_avg
            # loss_dict['loss_w'] = F.smooth_l1_loss(rec_ws, gt_w).mean() * lambda_w
