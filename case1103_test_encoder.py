...
@click.option('--encoder', 'encoder_pkl', help='Network pickle filename',
              default='./output/case1103_trunc075/debug/checkpoints/network-snapshot-000000.pkl')
    ...
    for idx in range(6):
        ...
        with torch.no_grad():
            if gen_img_w is not None:
                sample = torch.cat([img.detach(),gen_img_Sep.detach(),gen_img_w.detach()])
            else:
                sample = torch.cat([img.detach(), gen_img_Sep.detach()])
            utils.save_image(
                sample,
