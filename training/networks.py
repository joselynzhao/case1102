...
class Mapping_ws(torch.nn.Module):
    def __init__(self,n_w,mapping_way=1,device=None):
        ...
        model = torch.nn.Sequential(
            torch.nn.Linear(in_dim, mi_dim),
            torch.nn.LeakyReLU(inplace=True),
            torch.nn.Linear(mi_dim,out_dim)
        )
        self.device = device
...
@persistence.persistent_class
class Separate_net(torch.nn.Module):
    ...
    def get_model_mode(self):
        model_mode = torch.nn.Sequential(
    ...
    def forward(self,w):
        ...
        for i in range(n_dim):
            ...
            out_mode.append(self.mode_out[i](mode[i])) #512 2
        out_w = torch.stack(out_w,dim=0) # 17, b, 512
        out_mode = torch.stack(out_mode,dim=0) # 17, b, 2
        out_w = rearrange(out_w,"n b z -> b n z") # b 17 512
        out_mode = rearrange(out_mode,"n b z -> b n z") # b 17 2
        out_mode = torch.sum(out_mode,dim=(1,)) # b 2
